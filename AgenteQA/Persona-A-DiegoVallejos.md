...
- 2026-08-29: pruebas contra 4 sitios distintos (login, form multi-campo con nav pesada, API pura, SPA):
  - **Bug real encontrado y arreglado**: en `page_inspector.py`, el cap de `MAX_ELEMENTS=40` contaba `<a>` de navegacion con la misma prioridad que inputs/botones. En una pagina con menu grande (`demo.automationtesting.in/Register.html`, 43 links de nav), los 22 campos reales del formulario quedaban completamente afuera del resumen. Fix: separar `FORM_TAGS` (input/button/form/select/textarea, sin cap practico) de `NAV_TAGS` (`a`, solo llena el espacio que sobra hasta `MAX_ELEMENTS=60`). Test de regresion agregado (`test_nav_links_dont_crowd_out_form_elements`). Verificado: el plan generado post-fix uso selectores reales (`#firstpassword`, `#secondpassword`, `#submitbtn`, placeholders exactos).
  - API pura (jsonplaceholder.typicode.com/posts): plan bien tipeado 100% `endpoint`, con casos normales y de error (404, payload vacio).
